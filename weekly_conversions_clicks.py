...
def analyze_weekly_search_terms(
    ...
    """
    Головна функція для аналізу тижневих даних пошукових термінів.
    ...
    # Отримуємо частки кліків та конверсій з тижневої таблиці
    weekly_shares = get_weekly_shares(
        engine=engine,
    ...
    # Отримуємо сезонний фактор для тижня
    seasonal_factor = get_weekly_seasonal_factor(start_date, end_date)
...
def find_weekly_conversions(row: pd.Series, daily_total_orders: float = 0) -> Tuple[Dict[str, int], float]:
    """
    Розраховує метрики конверсій для тижня на основі рядка даних.
    Коефіцієнти збільшені в 7 разів у порівнянні з щоденними даними.
    
    :param row: рядок даних з часткою конверсій
    :param daily_total_orders: сума щоденних конверсій за тиждень (для обмеження)
    ...
    # Отримуємо сезонний фактор (передбачається, що він вже обчислений для тижня)
    seasonal_factor = 1.0
    ...
    delta = 0.025
    start = 7  # Збільшуємо стартове значення в порівнянні з щоденним
    
    # Визначення параметрів delta і start на основі SFR та min_share
    # Коефіцієнти збільшені в 7 разів у порівнянні з щоденними даними
    sfr_share_config = [
        # SFR range, min_share range, has_zero_share, delta, start
        ((0, 1700), (20, float('inf')), True, 0.005, 120),
        ((0, 1700), (8, 20), True, 0.005, 95),
        ((0, 1700), (2, 8), True, 0.005, 84),
        ((0, 1700), (1, 2), True, 0.005, 35),
        
        ((1700, 4000), (20, float('inf')), True, 0.01, 70),
        ((1700, 4000), (2, 20), True, 0.01, 40),
        ((1700, 4000), (1, 2), True, 0.01, 14),
        
        ((4000, 10000), (2, float('inf')), True, 0.01, 50),
        ((4000, 10000), (1, 2), True, 0.01, 14),
        
        ((10000, 40000), (2.5, float('inf')), True, 0.01, 14),
        
        ((40000, 100000), (9, float('inf')), True, 0.01, 14)
    ]
    
    # Спецкейс для високих значень min_share
    if any(share == 0 for share in conv_shares) and min_share >= 50:
        if row['SFR'] <= 10000:
            delta, start = 0.005, 210
        elif 10000 < row['SFR'] <= 30000:
            delta, start = 0.005, 140
        elif 30000 < row['SFR'] <= 70000:
            delta, start = 0.005, 70
    ...
    ab_lookup = [
        ...
        ((3500, 5000), 3290, 315),
        ((5000, 15000), 2765, 280),
        ((15000, 35000), 1890, 245),
        ((35000, 70000), 1540, 210),
        ((70000, 100000), 1120, 175),
        ((100000, 200000), 700, 140),
        ((200000, float('inf')), 350, 140)
    ]
    
    a, b = 1540, 210  # Дефолтні значення в 7 разів більші
...
def find_weekly_clicks(row: pd.Series, conv_row: pd.Series, daily_total_clicks: float = 0) -> Tuple[Dict[str, int], float]:
    """
    Розраховує метрики кліків для тижня на основі рядка даних та конверсій.
    Коефіцієнти збільшені в 7 разів у порівнянні з щоденними даними.
    
    :param row: рядок даних з часткою кліків
    :param conv_row: рядок даних з розрахованими конверсіями
    :param daily_total_clicks: сума щоденних кліків за тиждень (для обмеження)
    ...
    # Використовуємо щоденні дані, помножені на коефіцієнт, як обмеження
    min_click_threshold = daily_total_clicks * 0.6 * seasonal_factor
    ...
    click_params = [
        ...
        ((0, 100), 3500, 4, 0.02),
        ((100, 500), 3010, 3.8, 0.02),
        ((500, 1000), 2520, 3.6, 0.02),
        ((1000, 5000), 1820, 3.5, 0.025),
        ((5000, 20000), 1260, 3.3, 0.025),
        ((20000, 50000), 910, 2.7, 0.025),
        ((50000, 100000), 560, 2.0, 0.025),
        ((100000, 200000), 420, 1.5, 0.025),
        ((200000, float('inf')), 420, 1, 0.025)
    ]
    
    # Отримання параметрів з таблиці пошуку
    a, b, delta = 420, 1, 0.025  # Дефолтні значення в 7 разів більші
